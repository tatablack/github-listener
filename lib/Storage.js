...
_.extend(Storage.prototype, {
            ...
            r.dbList().run(conn, function(err, dbList) {
                ...
                if (!_.contains(dbList, this.databaseName)) {
                    r.dbCreate(this.databaseName).run(conn, function(err, result) {
                        ...
                        if (result.created) {
                            _.each(['payloads', 'installations'], function(tableName) {
    ...
    createInstallation: function(params) {
        'use strict';
        this.connection.use('github_listener');
        ...
        r.table('installations').insert(installation).run(this.connection, function(err, result) {
            if (err) {
                this.handleError(err);
                deferred.reject(new Error(err));
            } else {
                if (result.errors) {
                    this.log.warn('Unable to create the installation. The error reported is: %s', result.first_error);
                    deferred.reject(new Error(result.first_error));
                } else {
                    this.log.info('Installation saved successfully.');
                    deferred.resolve(installation);
                }                
            }
