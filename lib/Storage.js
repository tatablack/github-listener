...
_.extend(Storage.prototype, {
    ...
    getConnection: function(options) {
        this.log.info('Trying to get a new database connection');        
    ...
    savePayload: function(payload) {
        ...
        this.getConnection(DB_OPTIONS_EXTENDED).then(function(conn) {
            connection = conn;
            return r.table('payloads').insert(payload).run(connection);
        }).then(function(result) {
            if (result.errors) {
                log.warn('Unable to save the payload. The error reported is: %s', result.first_error);
                return false;
            } else {
                log.info('Payload saved successfully.');
                return true;
            }
        }).catch(function(err) {
            log.error({err: err}, 'A database error occurred.');
    ...
    getNotificationsFor: function(installationId) {
        ...
        return this.getConnection(DB_OPTIONS_EXTENDED).then(function(conn) {
            connection = conn;
            return r.table('payloads').concatMap(function(payload) {
                ...
                return payload('commits').filter(function(commit) {
                    return commit('mentions').contains(user('username')).or(
                        ...
                        })).and(payload('lastSeen').gt(user('lastSeen')));
                });
            }).run(connection);
