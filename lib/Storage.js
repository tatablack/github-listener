...
_.extend(Storage.prototype, {
    ...
    updateUser: function(params) {
        ...
        r.table('users').get(user.installationId).update(user).run(this.connection, function(err, result) {
            if (err) {
                this.handleError(err);
                deferred.reject(new Error(err));
            } else {
            ...
            return payload('commits').filter(function(commit) {
                return commit('mentions').contains(user('username')).or(
                    user('authors').contains(function(author) {
                        return commit('author')('name').eq(author).or(commit('author')('username').eq(author));
