...
var Storage = function(log) {
    ...
    this.log = log;
    this.connect = nodefn.lift(r.connect);
    this.dbList = nodefn.lift(r.dbList().run);
};
...
_.extend(Storage.prototype, {
    ...
    save: function(payload) {
        ...
        r.table('payloads').insert(payload).run(this.connection, function(result) {
            this.log.debug('Payload with id %s saved.', result.generated_keys[0]);
        }.bind(this));
    }
});
