...
function addGravatar(commit, configuration, log) {
    ...
    if (gitHubDomain) {
        var apiUrl = configuration.get('api')[gitHubDomain[1]],
            ...
            requestAvatarUrl = apiUrl + '/users/' + username;

        log.debug('Requesting avatar from: %s', requestAvatarUrl);

        return request(requestAvatarUrl).spread(function(response, body) {
...
_.extend(GitHubPayloadParser.prototype, {
    parse: function(rawPayload, deliveryHeader) {
        ...
        Promise.all(_.map(payload.commits, function(commit) {
            ...
            this.storage.saveCommits(payload.commits).then(function() {
                this.log.info('Received and saved %d commits from %s', payload.commits.length, payload.repository.name);
                this.storage.ee.emit('incomingPayload');
            }.bind(this));
